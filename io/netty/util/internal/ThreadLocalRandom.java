    ...
    for (;;)
    {
      ...
      if (seedUniquifier.compareAndSet(current, next))
      {
        if ((current == 0L) && (logger.isDebugEnabled())) {
          if (seedGeneratorEndTime != 0L) {
            logger.debug(String.format("-Dio.netty.initialSeedUniquifier: 0x%016x (took %d ms)", new Object[] { Long.valueOf(actualCurrent), Long.valueOf(TimeUnit.NANOSECONDS.toMillis(seedGeneratorEndTime - seedGeneratorStartTime)) }));
          } else {
