...
final class Bzip2HuffmanStageEncoder
{
  ...
  private static void generateHuffmanCodeLengths(int alphabetSize, int[] symbolFrequencies, int[] codeLengths)
  {
    ...
    for (int i = 0; i < alphabetSize; i++) {
      mergedFrequenciesAndIndices[i] >>>= 9;
    }
  ...
  private void optimiseSelectorsAndHuffmanTables(boolean storeSelectors)
  {
    ...
    for (int groupStart = 0; groupStart < mtfLength;)
    {
      ...
      for (int i = groupStart; i <= groupEnd; i++)
      {
        ...
        {
          int tmp107_105 = j; short[] tmp107_103 = cost;tmp107_103[tmp107_105] = ((short)(tmp107_103[tmp107_105] + huffmanCodeLengths[j][value]));
        }
      }
      byte bestTable = 0;
      int bestCost = cost[0];
      for (byte i = 1; i < totalTables; i = (byte)(i + 1))
      {
      ...
      for (int i = groupStart; i <= groupEnd; i++) {
        bestGroupFrequencies[mtfBlock[i]] += 1;
      }
      if (storeSelectors) {
