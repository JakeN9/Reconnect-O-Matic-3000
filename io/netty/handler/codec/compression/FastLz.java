...
final class FastLz
{
  ...
  static int compress(byte[] input, int inOffset, int inLength, byte[] output, int outOffset, int proposedLevel)
  {
    ...
    while (ip < ipLimit)
    {
      ...
      if (!matchLabel)
      {
        ...
        else
        {
          ...
          copy = 0;
          output[(outOffset + op++)] = 31; continue;
        }
        if (level == 2) {
          if (distance >= 8191L)
          {
      ...
      if (level == 2)
      {
        if (distance < 8191L)
        {
          if (len < 7)
          {
            output[(outOffset + op++)] = ((byte)(int)((len << 5) + (distance >>> 8)));
            output[(outOffset + op++)] = ((byte)(int)(distance & 0xFF));
          }
          else
          {
    ...
    if (level == 2)
    {
      int tmp1583_1581 = outOffset; byte[] tmp1583_1580 = output;tmp1583_1580[tmp1583_1581] = ((byte)(tmp1583_1580[tmp1583_1581] | 0x20));
    }
    return op;
  }
