...
public class Base64
{
  ...
  public static byte[] encode(byte[] source)
  {
    ...
    int d = 0;
    for (int e = 0; d < source.length - 2; e += 4)
    {
      encode3to4(source, d, 3, outBuff, e);d += 3;
    }
    if (d < source.length)
    {
  ...
  public static byte[] decode(byte[] source)
  {
    ...
    for (int i = 0; i < source.length; i++)
    {
      ...
      if (sbiDecode >= -5)
      {
        ...
        throw new IllegalArgumentException(String.format("Bad Base64 input character decimal %d in array position %d", new Object[] { Integer.valueOf(source[i] & 0xFF), Integer.valueOf(i) }));
      }
    }
